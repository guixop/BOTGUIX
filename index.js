...
const config = {
    A187: '📍GUIXBOT📍',
    instagram: '@guix_araujo',
    nomer: 'wa.me/85996283018',
    aktif: '24 JAM',
    youtube: 'GUIX OP',
    whatsapp: 'OFF',
   ...
   switch (prefix) {
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[AGUARDA AI TO FAZENDO ESSA P...', MessageType.text)
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   client.sendMessage(id, '[ESPERA UM POUCO TO FAZENDO ESSA P...', MessageType.text)
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui estão os acordes da música *${value}* querida(a) ♥️\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
  ...
  case 'cektanggal':
   ...
   case 'bitly':
           bitly(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui maninha(a) ja terminou☣️ :) \n\n${result} `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
